import { WireframeSpec, GenerateRequest, GenerateResponse, Settings, Model } from '../shared/types';
...
interface Message {
  type: 'generate' | 'render' | 'save-api-key' | 'get-settings' | 'load-models' | 'save-default-model' | 'error';
  data?: any;
}
...
figma.ui.onmessage = async (msg: Message) => {
  try {
    switch (msg.type) {
      ...
      case 'save-api-key':
        await handleSaveApiKey(msg.data as string);
        break;
      ...
      case 'save-default-model':
        await handleSaveDefaultModel(msg.data as string);
        break;
...
async function handleRender(spec: WireframeSpec) {
  ...
  // Render children with auto-mapping
...
async function renderChild(parent: FrameNode, child: any) {
  switch (child.type) {
    case 'text':
...
async function renderComponent(parent: FrameNode, child: any) {
  if (!child.componentName) {
    throw new Error('Component name is required');
  }
  
  // Auto-create components based on type
...
async function createAutoComponent(componentName: string, child: any): Promise<SceneNode | null> {
  await figma.loadFontAsync({ family: 'Inter', style: 'Regular' });
...
async function createButton(child: any, bgColor: string, textColor: string): Promise<FrameNode> {
  const frame = figma.createFrame();
  frame.name = 'Button';
  frame.x = child.x;
  frame.y = child.y;
...
async function createInput(child: any, isPassword: boolean): Promise<FrameNode> {
  const frame = figma.createFrame();
...
async function createTextElement(child: any, fontSize: number, fontWeight: string): Promise<TextNode> {
  const text = figma.createText();
  text.name = 'Text';
  ...
  if (child.width) text.resize(child.width, text.height);
  if (child.height) text.resize(text.width, child.height);
  
  return text;
}

async function createContainer(child: any, padding: number, bgColor: string): Promise<FrameNode> {
  const frame = figma.createFrame();
...
async function handleSaveApiKey(apiKey: string) {
  try {
    ...
    // Get user info from the validation response
...
async function handleLoadModels() {
  ...
  try {
    ...
    // Filter and sort models for wireframe generation
    const models = data.data
      .filter((model: any) => {
        // Filter for suitable models for wireframe generation
        return model.id && 
               model.name && 
               // Exclude vision, image, and specialized models
...
async function handleSaveDefaultModel(modelId: string) {
